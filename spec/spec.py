...
class Specification(YamlObject, yamltag="!spec", path_resolver=[]):
    ...
    def __init__(self, server: BaseLaunchableJar, plugins: Sequence[BaseJar], store: BaseStore, folders: FolderSpecification, **kw):
    ...
    @classmethod
    def from_yaml(cls, stream: str | IO) -> Specification:
        return load(stream)
...
class FolderSpecification(YamlObject, yamltag="!folder", path_resolver=["folders"]):
    ...
    def __init__(self, server: Path, plugins: Path, **kw):
